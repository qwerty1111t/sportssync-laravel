...
<link rel="stylesheet" href="{{ asset('auth.css') }}">
    ...
    <script>
        // Prevent bfcache/self-back navigation from restoring stale CSRF tokens.
        window.addEventListener('beforeunload', function () {});
        window.addEventListener('unload', function () {});
        window.addEventListener('pageshow', function (ev) {
            if ((ev && ev.persisted) || (performance && performance.getEntriesByType && performance.getEntriesByType('navigation').length && performance.getEntriesByType('navigation')[0].type === 'back_forward')) {
                ...
                try {
                    fetch(checkUrl, { credentials: 'same-origin', cache: 'no-store' })
                      ...
                      .then(function(js){
                          ...
                          var status = (js.status || '').toLowerCase();
