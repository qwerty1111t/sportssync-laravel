      ...
      fetch(url)
        ...
        .then(function(j) {
          if (j && j.success && j.state && !j.state._reset) {
            ...
            // Don't reset if we have local state to persist across refreshes
...
function render(state) {
  ...
  if (state.sets && state.sets.length > _prevSetsLength) {
    ...
    if (newSet && newSet.winner) {
      ...
      showWinnerModal('\u{1F3C6} SET WINNER', `${winnerName} wins Set ${newSet.setNumber}!`);
    }
  }
  ...
  if (state.winner_team && state.winner_team !== _prevWinnerTeam) {
    ...
    showWinnerModal('\u{1F3C6} MATCH WINNER', `${winnerName} wins the match!`);
...
function _loadStateFromDB() {
  try {
    ...
    if (window.MATCH_DATA && MATCH_DATA.match_id) {
      ...
      // Check localStorage for match_id to persist across refreshes
      try {
        const raw = localStorage.getItem(STORAGE_KEY);
        if (raw) {
          const parsed = JSON.parse(raw);
          if (parsed && parsed.match_id) {
            url = 'state.php?match_id=' + encodeURIComponent(parsed.match_id);
          }
        }
      } catch (_) {}
    }
    ...
    fetch(url)
      ...
      .then(function(j) {
        ...
        // Removed: else if (j && !j.success) { _resetIfLocalStateWasActive(); }
        // This prevents resetting the viewer on DB fetch failure, keeping localStorage state
      })
      .catch(function() { /* state.php unreachable — keep current state */ });
  } catch (_) {}
}
