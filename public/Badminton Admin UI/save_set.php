...
// optional committee/official
$committee = isset($data['committee_official']) ? trim($data['committee_official']) : (isset($data['committee']) ? trim($data['committee']) : null);
...
try {
    if (empty($match_id)) {
        if ($committee !== null) {
            $stmt = $mysqli->prepare("INSERT INTO badminton_matches (match_type, best_of, team_a_name, team_b_name, team_a_player1, team_a_player2, team_b_player1, team_b_player2, committee_official) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if ($stmt) {
                ...
                // fallback if column doesn't exist or prepare failed
                $stmt = $mysqli->prepare("INSERT INTO badminton_matches (match_type, best_of, team_a_name, team_b_name, team_a_player1, team_a_player2, team_b_player1, team_b_player2) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
