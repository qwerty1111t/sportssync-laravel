...
function bm_has_table(mysqli $db, string $table): bool {
...
if (!bm_has_table($mysqli, 'badminton_matches') || !bm_has_table($mysqli, 'badminton_sets')) {
  bm_json(['success' => false, 'message' => 'Badminton tables not created. Please run badminton_tables.sql'], 500);
}
...
$hasPlayers = bm_has_col($mysqli, 'badminton_matches', 'team_a_player1') && bm_has_col($mysqli, 'badminton_matches', 'team_a_player2') && bm_has_col($mysqli, 'badminton_matches', 'team_b_player1') && bm_has_col($mysqli, 'badminton_matches', 'team_b_player2');
...
try {
  ...
  $updateFields = ['match_type=?', 'best_of=?', 'team_a_name=?', 'team_b_name=?', 'status=?', 'winner_name=?'];
  $updateValues = [$matchType, $bestOf, $teamA, $teamB, $status, $winner];
  $types = 'sissss';
  if ($hasPlayers) {
    $updateFields = array_merge($updateFields, ['team_a_player1=?', 'team_a_player2=?', 'team_b_player1=?', 'team_b_player2=?']);
    $updateValues = array_merge($updateValues, [$pA1, $pA2, $pB1, $pB2]);
    $types .= 'ssss';
  }
  if (bm_has_col($mysqli, 'badminton_matches', 'committee_official')) {
    $updateFields[] = 'committee_official=?';
    $updateValues[] = $committee;
    $types .= 's';
  }
  $updateValues[] = $id;
  $types .= 'i';
  $sql = 'UPDATE badminton_matches SET ' . implode(', ', $updateFields) . ' WHERE id=?';
  $stmt = bm_stmt($mysqli, $sql);
  ...
  if (is_array($data['sets'] ?? null)) {
    ...
    if (bm_has_table($mysqli, 'badminton_match_summary')) {
      $stmt = bm_stmt($mysqli, 'SELECT id FROM badminton_match_summary WHERE match_id=? LIMIT 1');
      $stmt->bind_param('i', $id);
      $stmt->execute();
      $exists = $stmt->get_result()->fetch_assoc();
      $stmt->close();
      if ($exists) {
        $sum = bm_stmt($mysqli, 'UPDATE badminton_match_summary SET total_sets_played=?, team_a_sets_won=?, team_b_sets_won=?, winner_team=?, winner_name=? WHERE match_id=?');
        $sum->bind_param('iiissi', $played, $aWins, $bWins, $winnerTeam, $winner, $id);
      } else {
        $sum = bm_stmt($mysqli, 'INSERT INTO badminton_match_summary (match_id,total_sets_played,team_a_sets_won,team_b_sets_won,winner_team,winner_name) VALUES (?,?,?,?,?,?)');
  ...
  $mysqli->rollback();
  bm_json(['success' => false, 'message' => 'Unable to update match: ' . $e->getMessage()], 500);
}