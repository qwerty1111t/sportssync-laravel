...
function loadPersistedState() {
  try {
    ...
    setMatchType(state.matchType, { skipConfirm: true, skipSave: true, skipSync: true }); // prevent confirmation dialog during load
...
function syncSetWinCounts() {
  try {
    ...
    // Do NOT count current manual winner in gamesWon — only count saved sets
    // Manual winner is for UI preview only, not actual games won
  ...
  try {
    ...
    if (!state.swapped) {
      ...
      viewerState = {
        sets: Array.isArray(setHistory) ? setHistory.map(s => ({ setNumber: s.setNumber, teamAScore: s.teamAScore, teamBScore: s.teamBScore, winner: getSetWinner(s) })) : [],
        swapped: state.swapped || false,
        teamAName: state.teamB.name || 'TEAM B',
        teamBName: state.teamA.name || 'TEAM A',
        scoreA: state.teamB.score || 0,
        scoreB: state.teamA.score || 0,
        gamesA: state.teamB.gamesWon || 0,
        gamesB: state.teamA.gamesWon || 0,
        bestOf: state.bestOf || 3,
        currentSet: state.currentSet || 1,
        timeoutA: state.teamB.timeout || 0,
        timeoutB: state.teamA.timeout || 0,
        servingTeam: state.serving === 'A' ? 'B' : 'A',
        committee: committee || '',
        matchType: state.matchType || 'singles',
        teamAPlayer1: getPlayerInput('B', 0) || '',
        teamAPlayer2: getPlayerInput('B', 1) || '',
        teamBPlayer1: getPlayerInput('A', 0) || '',
        teamBPlayer2: getPlayerInput('A', 1) || '',
        manualWinners: state.manualWinners || {}
      };
    }
...
function toggleManualWinner(team) {
  try {
    ...
    try { updateAdminWinnerButtons(); } catch (_) {}
    saveLocalState(); // persist manual winner state
...
function saveMatch() {
  if (!confirm('⚠️ Warning: this will save the current match and create a new match. Continue?')) return;
  syncAllPlayerInputs();
  const payload = buildSavePayload();
  const committee = document.getElementById('bdCommitteeInput') ? document.getElementById('bdCommitteeInput').value.trim() : '';
  ...
  .then(j => {
    if (!j || !j.success) {
      throw new Error((j && j.message) ? j.message : 'Unknown error');
    }
    savedMatchId = j.match_id || sessionStorage.getItem('badminton_match_id');
    if (savedMatchId) {
      sessionStorage.setItem('badminton_match_id', savedMatchId);
    }
    const createPayload = {
      match_type:     state.matchType,
      best_of:        state.bestOf,
      team_a_name:    state.teamA.name || 'TEAM A',
      team_b_name:    state.teamB.name || 'TEAM B',
      team_a_player1: (state.teamA.players && state.teamA.players[0]) ? state.teamA.players[0] : null,
      team_a_player2: (state.teamA.players && state.teamA.players[1]) ? state.teamA.players[1] : null,
      team_b_player1: (state.teamB.players && state.teamB.players[0]) ? state.teamB.players[0] : null,
      team_b_player2: (state.teamB.players && state.teamB.players[1]) ? state.teamB.players[1] : null,
      committee_official: committee || null,
      sets: [{ set_number: 1, team_a_score: 0, team_b_score: 0, team_a_timeout_used: 0, team_b_timeout_used: 0, serving_team: 'A', set_winner: null }]
    };
    return fetch('save_set.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  .then(r => r.json())
  .then(newMatch => {
    if (!newMatch || !newMatch.success) {
      alert('Saved current match, but failed to create a new match: ' + (newMatch && newMatch.message ? newMatch.message : 'Unknown error'));
    } else {
      const newMatchId = String(newMatch.match_id);
      ...
      const freshViewerState = {
        ...
        servingTeam:  'A',
        committee:    committee,
        teamAPlayer1: (state.teamA.players && state.teamA.players[0]) ? state.teamA.players[0] : '',
        teamAPlayer2: (state.teamA.players && state.teamA.players[1]) ? state.teamA.players[1] : '',
        teamBPlayer1: (state.teamB.players && state.teamB.players[0]) ? state.teamB.players[0] : '',
        teamBPlayer2: (state.teamB.players && state.teamB.players[1]) ? state.teamB.players[1] : '',
        sets:         []
      };
      _broadcastNewMatch(newMatchId, freshViewerState);
    }
    if (savedMatchId) {
      window.location.href = 'badminton_report.php?match_id=' + encodeURIComponent(savedMatchId);
    }
  })
  .catch(err => { console.error(err); alert('Save failed: ' + err.message); })
  .finally(() => {
    if (btn) { btn.textContent = '⚠️ SAVE'; btn.disabled = false; }
  });
}
...
function confirmNewMatch() {
  ...
  .then(function(j) {
    ...
    setMatchType(state.matchType, { skipConfirm: true, skipSave: true, skipSync: true }); // prevent confirmation dialog during new match reset
