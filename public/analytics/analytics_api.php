    ...
    switch ($action) {
        ...
        case 'player_profile':
            ...
            $s=$pdo->prepare("SELECT bm.id AS match_id, bm.match_type, bm.team_a_name, bm.team_b_name,
                ...
                CASE WHEN bm.team_a_player1=" . $pdo->quote($n) . " OR bm.team_a_player2=" . $pdo->quote($n) . " THEN bm.team_a_name ELSE bm.team_b_name END AS player_team,
                CASE WHEN bm.team_a_player1=" . $pdo->quote($n) . " OR bm.team_a_player2=" . $pdo->quote($n) . " THEN 'A' ELSE 'B' END AS side
                ...
                ORDER BY bm.created_at ASC");
            $s->execute([':n5'=>$n,':n6'=>$n,':n7'=>$n,':n8'=>$n,':t'=>$t,':t2'=>$t]);
            $rows=$s->fetchAll();
            ...
            $s=$pdo->prepare("SELECT ttm.id AS match_id, ttm.match_type, ttm.team_a_name, ttm.team_b_name,
                ...
                CASE WHEN ttm.team_a_player1=" . $pdo->quote($n) . " OR ttm.team_a_player2=" . $pdo->quote($n) . " THEN ttm.team_a_name ELSE ttm.team_b_name END AS player_team,
                CASE WHEN ttm.team_a_player1=" . $pdo->quote($n) . " OR ttm.team_a_player2=" . $pdo->quote($n) . " THEN 'A' ELSE 'B' END AS side
            ...
            if (!$teamHistory) {
                ...
                $th=$pdo->prepare("SELECT 'badminton' AS sport,
                    CASE WHEN bm.team_a_name=" . $pdo->quote($t) . " AND (bm.team_a_player1=:n OR bm.team_a_player2=:n2) THEN 'A' ELSE 'B' END AS side,
                    " . $pdo->quote($t) . " AS team_name,
                ...
                $th->execute([':n'=>$n,':n2'=>$n,':n3'=>$n,':n4'=>$n,':n5'=>$n,':n6'=>$n,':t4'=>$t,':t5'=>$t]); foreach($th->fetchAll() as $r) $teamHistory[]=$r;
                // Table Tennis — FIX: same team-scoped filter
                $th=$pdo->prepare("SELECT 'table_tennis' AS sport,
                    CASE WHEN ttm.team_a_name=" . $pdo->quote($t) . " AND (ttm.team_a_player1=:n OR ttm.team_a_player2=:n2) THEN 'A' ELSE 'B' END AS side,
                    " . $pdo->quote($t) . " AS team_name,
