...
try {
    ...
    // FIX: race condition — serialize new-match creation so concurrent POSTs return the same match_id.
    $now = (int)(microtime(true) * 1000);
    $lockWindow = 5000;
    $matchId = null;
    try {
        $pdo->exec(
            "CREATE TABLE IF NOT EXISTS new_match_lock (
                sport VARCHAR(50) NOT NULL PRIMARY KEY,
                last_match_id INT UNSIGNED NOT NULL,
                last_created_at BIGINT NOT NULL
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
        );

        $pdo->beginTransaction();
        $lockStmt = $pdo->prepare('SELECT last_match_id, last_created_at FROM new_match_lock WHERE sport = :sport FOR UPDATE');
        $lockStmt->execute([':sport' => 'basketball']);
        $lockRow = $lockStmt->fetch(PDO::FETCH_ASSOC);

        if ($lockRow && ($now - (int)$lockRow['last_created_at']) < $lockWindow) {
            $matchId = (int)$lockRow['last_match_id'];
        } else {
            try {
                $stmt = $pdo->prepare(
                    'INSERT INTO `matches`
            ...
            $lockUpdate = $pdo->prepare(
                'INSERT INTO new_match_lock (sport, last_match_id, last_created_at) VALUES (:sport, :match_id, :ts) '
                . 'ON DUPLICATE KEY UPDATE last_match_id = :match_id, last_created_at = :ts'
            );
            $lockUpdate->execute([':sport' => 'basketball', ':match_id' => $matchId, ':ts' => $now]);
        }
        $pdo->commit();
    } catch (Exception $_e) {
        try { $pdo->rollBack(); } catch (Exception $_) {}
        // Locking failed; fallback to original insert behavior.
        try {
            $stmt = $pdo->prepare(
                'INSERT INTO `matches`
                    (team_a_name, team_b_name,
    ...
    if ($matchId === null) {
        throw new Exception('Unable to determine match_id');
    }
