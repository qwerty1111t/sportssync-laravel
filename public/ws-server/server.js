...
// Active basketball match room used to move fresh viewers from 'live' into the current admin room.
let activeBasketballMatchId = null;
...
// Build a read-only live snapshot for new/reloaded viewers.
// IMPORTANT: joining/reloading viewers must never mutate timer state.
// This only converts cached timer anchors into the current display value
// so clients do not subtract elapsed time a second time on reload.
function buildLiveSnapshot(cached) {
  try {
    if (!cached || !cached.payload) return cached && cached.payload ? cached.payload : cached;
    const now = Date.now();
    const payload = JSON.parse(JSON.stringify(cached.payload || {}));

    function liveTimer(t, fallbackTotal) {
      if (!t || typeof t !== 'object') return t;
      const total = typeof t.total === 'number' ? t.total : (typeof t.total_ms === 'number' ? t.total_ms / 1000 : fallbackTotal);
      const remaining = typeof t.remaining === 'number' ? t.remaining : (typeof t.remaining_ms === 'number' ? t.remaining_ms / 1000 : total);
      const running = !!t.running;
      const anchorTs = typeof t.start_timestamp === 'number' ? t.start_timestamp : (typeof t.ts === 'number' ? t.ts : (typeof cached.ts === 'number' ? cached.ts : now));
      const elapsed = running ? Math.max(0, (now - anchorTs) / 1000) : 0;
      const current = Math.max(0, Math.min(total, remaining - elapsed));
      return Object.assign({}, t, {
        total: total,
        remaining: Number(current.toFixed(3)),
    ...
    if (payload.gameTimer) payload.gameTimer = liveTimer(payload.gameTimer, 600);
    if (payload.shotClock) payload.shotClock = liveTimer(payload.shotClock, 24);
    if (payload.game_timer) payload.game_timer = liveTimer(payload.game_timer, 600);
    if (payload.shot_clock) payload.shot_clock = liveTimer(payload.shot_clock, 24);
    return payload;
  } catch (e) {
    return cached && cached.payload ? cached.payload : cached;
  }
}

function sendCachedStateTo(ws, matchId, cached, type) {
  if (!ws || !cached) return;
  const payload = buildLiveSnapshot(cached);
  sendRaw(ws, {
    type: type || 'applied_action',
    match_id: String(matchId),
    sport: cached && cached.sport ? cached.sport : 'basketball_state',
    payload: payload,
    ts: Date.now()
  });
}
  ...
  try {
    ...
    if (obj.type === 'join') {
      ...
      if (matchId) {
        ...
        // If viewer joined the fallback 'live' room, tell it the active basketball match id.
        if ((matchId === 'live' || matchId === '0') && activeBasketballMatchId && activeBasketballMatchId !== matchId) {
          try { sendRaw(sender, { type: 'active_match', match_id: activeBasketballMatchId, ts: Date.now() }); } catch (_) {}
          if (lastState.has(activeBasketballMatchId)) {
            const cachedActive = lastState.get(activeBasketballMatchId);
            try { sendCachedStateTo(sender, activeBasketballMatchId, cachedActive); } catch (_) {}
          }
        }
        if (lastState.has(matchId)) {
          const cached = lastState.get(matchId);
          sendCachedStateTo(sender, matchId, cached);
        }
      } else {
        sender._room = null;
      }
    ...
    // get_state message: send cached room state directly to requester.
    if (obj.type === 'get_state') {
      if (!sender) return;
      const targetId = (matchId === 'live' || matchId === '0') && activeBasketballMatchId ? activeBasketballMatchId : matchId;
      if (targetId && lastState.has(String(targetId))) {
        const cached = lastState.get(String(targetId));
        sendCachedStateTo(sender, String(targetId), cached);
      } else if (activeBasketballMatchId) {
        sendRaw(sender, { type: 'active_match', match_id: activeBasketballMatchId, ts: Date.now() });
      }
      return;
    }
    ...
    // timer_update message: broadcast within room so all clients (admins + viewers) see timer changes
    if (obj.type === 'timer_update' && matchId) {
      try { if (matchId !== 'live' && matchId !== '0') activeBasketballMatchId = matchId; } catch (_) {}
      try {
      ...
      // Broadcast to all clients in the room, not just admins
    ...
    // Also includes timer_update so it broadcasts to ALL clients (admins + viewers)
    const STATE_TYPES = ['state','badminton_state','tabletennis_state','basketball_state','basketball:state-sync','timer_update','admin_present'];
    if (STATE_TYPES.includes(obj.type) && matchId) {
      try { if ((obj.type === 'basketball_state' || obj.type === 'basketball:state-sync' || obj.type === 'admin_present' || obj.sport === 'basketball') && matchId !== 'live' && matchId !== '0') activeBasketballMatchId = matchId; } catch (_) {}
      try {
    ...
    try {
      ...
      if (obj.type === 'join') {
        ...
        if (matchId) {
          ...
          // If viewer joined the fallback 'live' room, tell it the active basketball match id.
          if ((matchId === 'live' || matchId === '0') && activeBasketballMatchId && activeBasketballMatchId !== matchId) {
            try { sendRaw(ws, { type: 'active_match', match_id: activeBasketballMatchId, ts: Date.now() }); } catch (_) {}
            if (lastState.has(activeBasketballMatchId)) {
              const cachedActive = lastState.get(activeBasketballMatchId);
              try { sendCachedStateTo(ws, activeBasketballMatchId, cachedActive); } catch (_) {}
            }
          }
          // send last cached state for this room (if any)
          if (lastState.has(matchId)) {
            const cached = lastState.get(matchId);
            sendCachedStateTo(ws, matchId, cached);
          }
        } else {
          ws._room = null;
        }
      ...
      if (obj.type === 'get_state') {
        ...
        if (targetId && lastState.has(String(targetId))) {
          const cached = lastState.get(String(targetId));
          sendCachedStateTo(ws, String(targetId), cached);
        } else if (activeBasketballMatchId) {
          sendRaw(ws, { type: 'active_match', match_id: activeBasketballMatchId, ts: Date.now() });
        }
        return;
      }
