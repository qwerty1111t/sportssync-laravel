...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($match_id > 0) {
        ...
        // ✅ NEW: Write current_match_id.json so viewers polling match_id=0 can find the active match
        $currentMatchFile = __DIR__ . '/current_match_id.json';
        $currentMatchData = json_encode(['match_id' => $match_id, 'updated_at' => date('c')], JSON_UNESCAPED_UNICODE);
        $cmWritten = @file_put_contents($currentMatchFile, $currentMatchData, LOCK_EX);
        error_log('[state.php POST] current_match_id.json written: ' . ($cmWritten ? 'YES' : 'NO') . ' match_id=' . $match_id);
    } else {
