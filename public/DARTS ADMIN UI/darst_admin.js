...
function publishLiveState() {
  ...
  const payload = { match_id: matchId, state: stateObj, client_id: _clientId };
  console.log('[publishLiveState] Admin sending to state.php: match_id=' + matchId + ' players.length=' + (stateObj.players ? stateObj.players.length : 0) + ' currentLeg=' + stateObj.currentLeg + ' currentPlayer=' + stateObj.currentPlayer);
  console.log('[publishLiveState] Full state keys:', Object.keys(stateObj).join(', '));
  fetch('state.php', { method: 'POST', credentials: 'include', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) })
    ...
    .then(data => {
      ...
      if (data && data.success) {
        console.log('[publishLiveState] ✅ Server persistence successful for match_id=' + matchId);
  ...
  const resetState = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.log('[newMatch] ✅ Match created with id=' + matchId);
        // Now publish the fresh blank state with the new match_id
        const freshState = serializeLiveState();
        console.log('[newMatch] Publishing fresh state to state.php with matchId=' + matchId + ' players=' + (freshState.players ? freshState.players.length : 0));
        const _nm_payload = { match_id: matchId || 0, state: freshState };
        fetch('state.php', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(_nm_payload) })
          .then(() => { 
            console.log('[newMatch] ✅ State published to state.php for matchId=' + matchId);
            try { broadcastCanonicalState(freshState); } catch(e) {} 
          })
          .catch(err => {
            console.error('[newMatch] ❌ Failed to publish state:', err);
          });
      }
    } catch (e) {
      console.error('[newMatch] ❌ Failed to create new match:', e);
    }
  };
