...
function renderState(st) {
  ...
  console.log('[Darts Viewer] renderState match_id:', st.match_id != null ? String(st.match_id) : 'null', 'currentLeg:', st.currentLeg, 'updated_at:', st.updated_at);
  ...
  if (waiting) waiting.style.display = hasMatch ? 'none' : '';
  if (grid)    grid.style.display    = hasMatch ? '' : 'none';
...
function connectWS() {
  ...
  ws.addEventListener('open', function() {
    ...
    console.log('[Darts Viewer] WS open, joining match_id:', mid);
...
function pollOnce() {
  ...
  console.log('[Darts Viewer] polling match_id:', mid);
  ...
  fetch(url, { cache: 'no-store' })
    ...
    .then(function(data) {
      ...
      if (st) {
        console.log('[Darts Viewer] Polled state from', url, '— players:', st.players ? st.players.length : 0, 'currentLeg:', st.currentLeg, 'gameType:', st.gameType);
        renderState(st);
      } else {
        console.log('[Darts Viewer] Poll returned null state from', url);
      }
      setConnStatus('poll', 'LIVE');
    })
...
(function init() {
  ...
  fetch('state.php?match_id=0&t=' + Date.now(), { cache: 'no-store' })
    ...
    .then(function(data) {
      var st = data && data.state ? data.state : (data && data.payload ? data.payload : null);
      if (st) {
        console.log('[Darts Viewer] Initial state load — match_id:', st.match_id != null ? String(st.match_id) : 'null', 'players:', st.players ? st.players.length : 0, 'gameType:', st.gameType, 'currentLeg:', st.currentLeg);
        console.log('[Darts Viewer] Initial state keys:', Object.keys(st).join(', '));
