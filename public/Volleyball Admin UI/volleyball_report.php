...
<div class="export-bar">
  ...
  <button class="btn-export" onclick="saveMatchData()">&#128190; Save Match</button>
...
window.saveMatchData = async function () {
  try {
    const normalizePlayers = (players) => (players || []).map(p => ({
      no: p.jersey_no || '',
      name: p.player_name || '',
      pts: p.pts || 0,
      spike: p.spike || 0,
    ...
    const payload = {
      teamA: {
        name: MATCH_DATA.team_a_name || 'TEAM A',
        score: MATCH_DATA.team_a_score || 0,
        timeout: 0,
        players: normalizePlayers(MATCH_DATA.players_a)
      },
      teamB: {
        name: MATCH_DATA.team_b_name || 'TEAM B',
        score: MATCH_DATA.team_b_score || 0,
        timeout: 0,
        players: normalizePlayers(MATCH_DATA.players_b)
      },
      shared: { set: MATCH_DATA.current_set || 1 },
      committee: MATCH_DATA.committee || ''
    };
    ...
    if (data && data.success) {
      alert('Match saved to database with ID ' + data.match_id + '.');
      return;
    }
...
<script>
let _volleyballReportWarnOnLeave = true;
window.addEventListener('beforeunload', function (e) {
  if (!_volleyballReportWarnOnLeave) return;
  const message = 'If you leave this report page without creating a new match, current data may be lost. Create a new match to save the game.';
  e.preventDefault();
...
function newMatch(){
  _volleyballReportWarnOnLeave = false;
  try { localStorage.removeItem('volleyballMatchState'); } catch(e){}
  try { localStorage.removeItem('volleyballAdminState'); } catch(e){}
  try { sessionStorage.removeItem('volleyball_match_id'); } catch(e){}
