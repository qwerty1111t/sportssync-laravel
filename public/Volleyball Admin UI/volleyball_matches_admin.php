...
<title>Volleyball Matches — SportSync Admin</title>
...
.ss-brand{
  ...
  letter-spacing:0.06em;color:#e8eaed;white-space:nowrap;
}
.ss-bolt{color:var(--primary);font-size:1em}
...
.ss-theme-btn{
  ...
  color:#e8eaed;padding:7px 10px;border-radius:var(--radius);
...
.ss-input{
  ...
  padding:10px 14px;transition:var(--tr);
  min-height:42px;
}
...
.ss-badge{
  ...
  font-family:var(--font-head);font-size:0.7rem;letter-spacing:0.08em;
...
.ss-badge-loss{background:rgba(204,51,51,0.12);color:#f87171;border:1px solid rgba(204,51,51,0.25)}
...
.ss-pg-ellipsis{color:var(--text-dim);padding:0 4px;font-size:0.85rem}
...
<main class="ss-main">
  <div class="ss-page-header">
    <h1 class="ss-page-title">🏐 Volleyball Matches</h1>
...
?>
          <tr id="match-row-<?= (int)$m['match_id'] ?>">
            ...
            <td class="js-score"><strong><?= (int)$m['team_a_score'] ?></strong> <span style="color:var(--text-dim)">–</span> <strong><?= (int)$m['team_b_score'] ?></strong></td>
            <td class="js-period"><?= (int)$m['current_set'] ?></td>
            <td class="js-result"><span class="<?= $badgeCls ?>"><?= $res ?></span></td>
            <td class="js-committee" style="color:var(--text-muted)"><?= htmlspecialchars($m['committee'] ?? '') ?></td>
            <td style="color:var(--text-muted);font-size:0.82rem;white-space:nowrap"><?= htmlspecialchars($m['created_at']) ?></td>
            <td style="white-space:nowrap">
              <a class="ss-btn-link" href="volleyball_report.php?match_id=<?= (int)$m['match_id'] ?>" target="_blank">Report</a>
              <button type="button" class="ss-btn ss-btn-sm ss-btn-ghost" onclick="openMatchEdit(<?= (int)$m['match_id'] ?>)">Edit</button>
              <button type="button" class="ss-btn ss-btn-sm ss-btn-danger" onclick="resetMatch(<?= (int)$m['match_id'] ?>)">Reset</button>
            </td>
          </tr>
