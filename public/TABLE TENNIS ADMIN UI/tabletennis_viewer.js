...
function _buildViewerResetState(payload) {
  payload = payload || {};
  // ✅ PERSISTENCE FIX: Preserve team names and committee from payload if present
  // Try to get team names from payload nested structure
  const payloadTeamAName = payload.teamA && payload.teamA.name ? payload.teamA.name : (payload.teamAName || 'TEAM A');
  const payloadTeamBName = payload.teamB && payload.teamB.name ? payload.teamB.name : (payload.teamBName || 'TEAM B');
  const payloadCommittee = payload.committee || payload.committee_official || '';
  return {
    _resetApplied: true,
    ...
    bestOf: 3,
    currentSet: 1,
    committee: payloadCommittee,
    teamA: { name: payloadTeamAName, score: 0, gamesWon: 0, timeout: 0, players: [''] },
    teamB: { name: payloadTeamBName, score: 0, gamesWon: 0, timeout: 0, players: [''] },
    sets: [],
    manualWinners: {}
  };
}
...
function render(state) {
  ...
  // ✅ PERSISTENCE FIX: Auto-save state to localStorage for cross-reload persistence
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(Object.assign({}, state, { _savedAt: new Date().toISOString() })));
  } catch (_) {}
  ...
  // ✅ PERSISTENCE FIX: Ensure committee is never empty by preserving from previous state if needed
  let committee = (state.committee || state.committee_official || '') || (tA.committee || '');
  // Store in state for persistence
  if (committee) state.committee = committee;
  ...
  // Swap layout
  // ✅ SWAP TEAMS: Use CSS order property (flexbox layout uses order, not gridTemplateAreas)
  const panelA = document.getElementById('panelA');
  const panelB = document.getElementById('panelB');
  const toRow  = document.getElementById('timeoutRow');
  if (state.swapped) {
    // When swapped: panelA moves right (order 3), panelB moves left (order 1)
    if (panelA) panelA.style.order = '3';
    if (panelB) panelB.style.order = '1';
    if (toRow) toRow.style.flexDirection = 'row-reverse';
  } else {
    // When not swapped: panelA on left (order 1), panelB on right (order 3)
    if (panelA) panelA.style.order = '1';
    if (panelB) panelB.style.order = '3';
