...
<div class="export-bar" style="background:#b71c1c;border-bottom:3px solid #FFCDD2;padding:10px 24px;display:flex;align-items:center;gap:10px;">
  ...
  <div style="margin-left:auto;display:flex;gap:8px;align-items:center">
    <button class="btn-export btn-excel" onclick="saveMatchData()">💾 Save Match</button>
...
<script>
window.saveMatchData = function saveMatchData() {
  if (!confirm('Save match data to database?')) return;

  // Build payload in the format expected by save_set.php (no match_id to create new match)
  const payload = {
    match_type: MATCH_DATA.match_type,
    best_of: MATCH_DATA.best_of,
    team_a_name: MATCH_DATA.team_a_name,
    team_b_name: MATCH_DATA.team_b_name,
    team_a_player1: (MATCH_DATA.players_a && MATCH_DATA.players_a[0]) ? MATCH_DATA.players_a[0].name : '',
    team_a_player2: (MATCH_DATA.players_a && MATCH_DATA.players_a[1]) ? MATCH_DATA.players_a[1].name : '',
    team_b_player1: (MATCH_DATA.players_b && MATCH_DATA.players_b[0]) ? MATCH_DATA.players_b[0].name : '',
    team_b_player2: (MATCH_DATA.players_b && MATCH_DATA.players_b[1]) ? MATCH_DATA.players_b[1].name : '',
    committee_official: MATCH_DATA.committee,
    sets: MATCH_DATA.sets,
    winner_name: MATCH_DATA.overall_winner
  };
  ...
  .then(j => {
    if (j && j.success) {
      ...
      // Redirect to the new match report
      window.location.href = 'tabletennis_report.php?match_id=' + j.match_id;
    } else {
