...
$original_match_id = $match_id; // Track if this is a new match
...
if ($match_id !== null) {
    ...
    if ($chk->execute()) {
        $chk->store_result();
        if ($chk->num_rows === 0) { $match_id = null; } // Treat as new if not found
    }
    $chk->close();
}
...
try {
    ...
    if (!empty($data['sets']) && is_array($data['sets'])) {
        ...
        $is_new = ($original_match_id === null);
        $type = $is_new ? 'new_match' : 'tabletennis_state';
        try {
            $emitPayload = [
                'match_id' => $match_id,
            ...
            emit_ws(['type' => $type, 'match_id' => $match_id, 'sport' => 'tabletennis', 'payload' => $emitPayload]);
        } catch (Throwable $_) { /* non-fatal */ }
