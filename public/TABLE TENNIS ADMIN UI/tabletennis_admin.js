...
function swapTeams() {
  ...
  // ✅ SWAP NOTIFICATION: Display which teams are being swapped
  (function() {
    const teamAName = state.teamA.name || 'TEAM A';
    const teamBName = state.teamB.name || 'TEAM B';
    const swapStatus = state.swapped ? 'SWAPPED' : 'RESET';
    const toast = document.createElement('div');
    toast.textContent = '⇄ ' + teamAName + ' ⇄ ' + teamBName + ' — ' + swapStatus;
    toast.style.cssText = 'position:fixed;bottom:70px;left:50%;transform:translateX(-50%);background:#1a5c7c;color:#fff;padding:10px 20px;border-radius:8px;font-weight:700;font-size:13px;z-index:99999;box-shadow:0 4px 16px rgba(0,0,0,.5);white-space:nowrap';
...
function saveAndReport() {
  ...
  .then(j => {
    if (j && j.success) {
      ...
      try {
        if (_ttBC) _ttBC.postMessage({ _newMatchId: matchId });
      } catch (_) {}
...
try {
  if (_ttBC) {
    _ttBC.addEventListener('message', function(e) {
      ...
      if (e.data && e.data._newMatchId) {
        sessionStorage.setItem('tabletennis_match_id', e.data._newMatchId);
      }
    });
  }
} catch (_) {}
