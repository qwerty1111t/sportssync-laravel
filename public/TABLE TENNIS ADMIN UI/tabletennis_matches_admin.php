...
$where = [];
if ($q !== '') $where[] = "(id = ? OR team_a_name LIKE ? OR team_b_name LIKE ?)";
if ($statusFilter !== '') $where[] = "status = ?";
...
if ($stmt) {
    $bindTypes = '';
    $bindVals = [];
    if ($q !== '') { $bindTypes .= 'iss'; $bindVals[] = intval($q); $like = '%'.$q.'%'; $bindVals[] = $like; $bindVals[] = $like; }
    ...
    $res = $mysqli->query('SELECT id, match_type, best_of, team_a_name, team_b_name, status, winner_name, created_at FROM table_tennis_matches ORDER BY created_at DESC LIMIT 200');
...
<title>Table Tennis Matches — SportSync Admin</title>
...
.ss-pg-btn{display:inline-flex;align-items:center;justify-content:center;min-width:34px;height:34px;padding:0 8px;font-family:var(--font-head);font-size:0.78rem;font-weight:600;letter-spacing:0.04em;background:var(--surface-alt);color:var(--text-muted);border:1px solid var(--border-s);border-radius:var(--radius);cursor:pointer;transition:var(--tr);text-decoration:none}
...
<nav class="ss-nav">
  <div class="ss-nav-inner">
    ...
    <div class="ss-breadcrumb">
      ...
      <span class="ss-bc-current">Table Tennis Matches</span>
    </div>
...
<main class="ss-main">
  <div class="ss-page-header">
    <h1 class="ss-page-title">🏓 Table Tennis Matches</h1>
  ...
  <div class="ss-card">
    <!-- Pagination Top (populated by JS) -->
...
?>
          <tr id="match-row-<?= (int)$m['id'] ?>" class="data-row">
            ...
            <td style="white-space:nowrap">
              <a class="ss-btn-link" href="tabletennis_report.php?match_id=<?= (int)$m['id'] ?>" target="_blank">Report</a>
...
<?php endforeach; endif; ?>
    ...
    <!-- Pagination Bottom (populated by JS) -->
...
<!-- Client-side Pagination (new display layer — existing JS logic untouched) -->
<script>
(function(){
  var PER_PAGE = 10;
