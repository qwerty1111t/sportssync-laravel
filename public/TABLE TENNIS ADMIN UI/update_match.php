...
function tt_has_table(mysqli $db, string $table): bool {
...
if (!tt_has_table($mysqli, 'table_tennis_matches') || !tt_has_table($mysqli, 'table_tennis_sets')) {
  tt_json(['success' => false, 'message' => 'Table Tennis tables not created. Please run table_tennis_tables.sql'], 500);
}
...
try {
  ...
  if (is_array($data['sets'] ?? null)) {
    ...
    if (tt_has_table($mysqli, 'table_tennis_match_summary')) {
      $stmt = tt_stmt($mysqli, 'SELECT id FROM table_tennis_match_summary WHERE match_id=? LIMIT 1');
      $stmt->bind_param('i', $id);
      $stmt->execute();
      $exists = $stmt->get_result()->fetch_assoc();
      $stmt->close();
      if ($exists) {
        $sum = tt_stmt($mysqli, 'UPDATE table_tennis_match_summary SET total_sets_played=?, team_a_sets_won=?, team_b_sets_won=?, winner_team=?, winner_name=? WHERE match_id=?');
        $sum->bind_param('iiissi', $played, $aWins, $bWins, $winnerTeam, $winner, $id);
      } else {
        $sum = tt_stmt($mysqli, 'INSERT INTO table_tennis_match_summary (match_id,total_sets_played,team_a_sets_won,team_b_sets_won,winner_team,winner_name) VALUES (?,?,?,?,?,?)');
