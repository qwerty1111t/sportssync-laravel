...
// Check if the match has a winner (completed)
$checkStmt = $mysqli->prepare("SELECT winner_name FROM table_tennis_matches WHERE id=?");
$checkStmt->bind_param('i', $match_id);
$checkStmt->execute();
$result = $checkStmt->get_result();
$row = $result->fetch_assoc();
$hasWinner = $row && !empty($row['winner_name']);
$checkStmt->close();

$status = $hasWinner ? 'completed' : 'reset';

$stmt = $mysqli->prepare("UPDATE table_tennis_matches SET status=?, winner_name=NULL WHERE id=?");
...
// Clear live-state cache so viewers using state.php cannot reload stale pre-reset data.
$mysqli->query("
...
$live_state_id = 'live';
$stmt = $mysqli->prepare("DELETE FROM table_tennis_match_state WHERE match_id IN (?, ?)");
$stmt->bind_param('ss', $match_state_id, $live_state_id);
if (!$stmt->execute()) { http_response_code(500); echo json_encode(['success' => false, 'message' => $stmt->error]); exit; }
$stmt->close();

// Notify WS relay about reset (best-effort)
try {
	$wsRelay = getenv('WS_RELAY_URL') ?: 'http://127.0.0.1:3000/emit';
	$wsToken = getenv('WS_TOKEN') ?: null;
	$emit = json_encode(['type' => 'new_match', 'match_id' => $match_id, 'sport' => 'tabletennis', 'payload' => ['match_id' => $match_id, '_reset' => true]]);
