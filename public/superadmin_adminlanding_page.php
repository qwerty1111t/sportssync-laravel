<?php
// ============================================================
// superadmin_adminlanding_page.php — SportSync Admin Landing Page
// Laravel-compatible version with fixed URLs
// ============================================================

// ── BOOTSTRAP: Define helpers BEFORE loading legacy auth ──────────────────────
...
$user = currentUser();
 if (!$user) {
  // Redirect direct web access to the Laravel superadmin login
  header('Location: ' . (function_exists('url') ? url('/superadmin/login') : '/superadmin/login') . '?next=adminlanding'); exit;
 }
...
<script>
  // CSRF token for API calls - use Laravel's csrf_token()
  window.CSRF_TOKEN = '<?php echo htmlspecialchars(csrf_token()); ?>';
  console.log('CSRF Token loaded:', window.CSRF_TOKEN ? 'Present (' + window.CSRF_TOKEN.length + ' chars)' : 'EMPTY');
</script>
...
<nav class="navbar" id="navbar">
  <div class="nav-container">
    <a href="<?php echo e(url('/')); ?>" class="nav-logo">
      <img src="<?php echo e(asset('logo.png')); ?>" alt="SportSync" class="logo-img">
    </a>
    <ul class="nav-links" id="nav-links">
      <li><a href="<?php echo e(url('/superadmin/landing')); ?>" class="nav-link">Home</a></li>
      <li><a href="<?php echo e(url('/')); ?>" class="nav-link active">Dashboard</a></li>
    </ul>
    <div class="nav-auth">
      ...
      <a href="<?php echo e(url('/superadmin/logout')); ?>" class="nav-auth-btn nav-logout">Logout</a>
    </div>
...
<div class="admin-wrap">
  ...
  <aside class="admin-sidebar" id="adminSidebar">
    ...
    <div class="sidebar-section">
      ...
      <nav class="sidebar-nav">
        <a href="<?php echo e(url('/badminton/admin')); ?>" class="sidebar-link">
          <span class="s-icon">🏸</span> Badminton Matches
        </a>
        <a href="<?php echo e(url('/basketball/admin')); ?>" class="sidebar-link">
          <span class="s-icon">🏀</span> Basketball Matches
        </a>
        <a href="<?php echo e(url('/darts/admin')); ?>" class="sidebar-link">
          <span class="s-icon">🎯</span> Darts Matches
        </a>
        <a href="<?php echo e(url('/tabletennis/admin')); ?>" class="sidebar-link">
          <span class="s-icon">🏓</span> Table Tennis Matches
        </a>
        <a href="<?php echo e(url('/volleyball/admin')); ?>" class="sidebar-link">
    ...
    <div class="admin-page" id="page-feedback">
      <div class="admin-page-header">
        <div>
          ...
          <p class="admin-breadcrumb">SportSync / Admin / <strong>Feedback</strong></p>
        </div>
        <span style="font-family:var(--font-head);font-size:0.82rem;color:var(--gray);">All submissions</span>
      </div>

      <div class="toolbar">
        <input type="text" class="search-input" placeholder="🔍 Search by name or email…" id="feedbackSearch">
        <select class="filter-select" id="statusFilter">
          <option value="">All Status</option>
...
<script src="<?php echo e(asset('landingpage.js')); ?>"></script>

<script>
'use strict';

/* ── Laravel-compatible URLs for JavaScript ── */
window.__SS_LOGOUT_URL = '<?php echo e(url("/superadmin/logout")); ?>';
window.__SS_BASE_URL = '<?php echo e(url("/")); ?>';
      ...
      try {
        ...
        if (currentUid && String(currentUid) === String(uid)) {
          ...
          if (String(status).toLowerCase() !== 'approved' && location.pathname.indexOf('adminlanding_page.php') === -1) {
            try { window.location.replace(window.__SS_LOGOUT_URL || '/superadmin/logout'); } catch (e) { window.location.reload(); }
          }
        }
      } catch (e) {}
...
/* ── AJAX helper using Laravel routes ── */
...
/* Legacy AJAX helper for self-submitting forms */
...
async function toggleUserStatus(uid) {
  ...
  try {
    const d = await apiPost('/superadmin/admin-landing/toggle-user-status', { user_id: uid });
...
async function approveApplicant(uid) {
  ...
  try {
    const d = await apiPost('/superadmin/admin-landing/approve-reject', { user_id: uid, action: 'approve' });
...
async function rejectApplicant(uid) {
  ...
  try {
    const d = await apiPost('/superadmin/admin-landing/approve-reject', { user_id: uid, action: 'reject' });
    if (!d.success) { alert('Error: ' + (d.message || 'Unknown')); return; }
    const row = document.getElementById('app-' + uid);
    if (row) row.remove();
    const btn = document.getElementById('pendingApplicantsBtn');
...
async function deleteUser(uid, uname) {
  ...
  try {
    const d = await apiPost('/superadmin/admin-landing/delete-user', { user_id: uid });
    if (!d.success) { alert('Error: ' + (d.message || 'Unknown')); return; }
    const row = document.getElementById('urow-' + uid);
    if (row) row.remove();
...
async function saveUsername(uid) {
  ...
  try {
    const d = await apiPost('/superadmin/admin-landing/change-username', { user_id: uid, new_username: val });
    if (!d.success) {
...
async function submitAddUser() {
  ...
  try {
    const d = await apiPost('/superadmin/admin-landing/add-user', { username, password, role });
...
async function toggleSportStatus(spId) {
  ...
  try {
    const d = await apiPost('/superadmin/admin-landing/toggle-sport-status', { sport_id: spId });
    if (!d.success) {
...
async function toggleMaintenanceMode(checked) {
  ...
  try {
    const d = await apiPost('/superadmin/admin-landing/save-setting', { key: 'maintenance_mode', value });
    if (!d.success) {
...
async function loadFeedback() {
  try {
    ...
    document.getElementById('feedbackTbody').innerHTML = '<tr><td colspan="6" class="dt-empty">Error loading feedback</td></tr>';
  }
}
...
function displayFeedback(feedbacks) {
  ...
  if (!feedbacks || feedbacks.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" class="dt-empty">No feedback yet.</td></tr>';
    document.getElementById('feedbackCountLabel').textContent = '0 Total';
    return;
  }
  ...
  tbody.innerHTML = feedbacks.map((f, i) => {
    ...
    const statusBg = statusColors[f.status] || '#1565C0';
    const dateObj = new Date(f.created_at);
    const dateStr = dateObj.toLocaleDateString() + ' ' + dateObj.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
    
    return `
      <tr id="feedback-row-${f.id}" data-status="${f.status}">
        <td><strong>${esc(f.name)}</strong></td>
        <td><a href="mailto:${esc(f.email)}" style="color:var(--yellow);text-decoration:none;">${esc(f.email)}</a></td>
        <td style="max-width:400px;color:var(--gray-light);word-break:break-word;">${esc(f.message).substring(0, 100)}${f.message.length > 100 ? '...' : ''}</td>
        <td><span class="badge" style="background:${statusBg}20;color:${statusBg};border:1px solid ${statusBg};">${f.status.toUpperCase()}</span></td>
        <td style="color:var(--gray);font-size:0.78rem;white-space:nowrap;">${dateStr}</td>
        <td>
          <select class="filter-select" style="font-size:0.75rem;padding:4px 8px;" onchange="updateFeedbackStatus(${f.id}, this.value)">
  ...
  document.getElementById('feedbackCountLabel').textContent = feedbacks.length + ' Total';
}
...
async function updateFeedbackStatus(feedbackId, status) {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (row) {
        row.style.background = 'rgba(0, 200, 83, 0.1)';
...
// Load feedback when page is navigated to
...
// Initial load if feedback page exists
if (document.getElementById('page-feedback')) {
  loadFeedback();
}

// Setup search and filter for feedback
setTimeout(() => {
...
function filterFeedback() {
  ...
  const rows = document.querySelectorAll('#feedbackTbody tr[id^="feedback-row-"]');
