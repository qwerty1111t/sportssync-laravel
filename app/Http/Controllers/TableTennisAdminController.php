...
class TableTennisAdminController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            Log::info('[TABLE TENNIS] Starting request', ['path' => $legacyPath, 'user' => auth()->id()]);
            
            if (!file_exists($legacyPath)) {
                Log::error('[TABLE TENNIS] File not found', ['path' => $legacyPath]);
            ...
            include $legacyPath;
            $html = ob_get_clean();
            Log::info('[TABLE TENNIS] Legacy HTML loaded', ['length' => strlen($html)]);
            ...
            Log::error('[TABLE TENNIS] Exception: ' . $e->getMessage(), ['exception' => $e]);
